...
class LogRevisionsListener implements EventSubscriber
{
    ...
    private function saveRevisionEntityData($class, $entityData, $revType)
    {
        if ($class->inheritanceType == ClassMetadata::INHERITANCE_TYPE_JOINED) {
            $entityDataForClass = $this->extractEntityDataForClass($class, $entityData);
            $this->saveRevisionEntityDataForChild($class, $entityDataForClass, $revType);
            foreach ($class->parentClasses as $parentClassName) {
                $parentClass = $this->em->getClassMetadata($parentClassName);
                $entityDataForClass = $this->extractEntityDataForClass($parentClass, $entityData);
                $this->saveRevisionEntityDataForChild($parentClass, $entityDataForClass, $revType);
            }
        } else {
            $this->saveRevisionEntityDataForChild($class, $entityData, $revType);
        }
    }

    private function saveRevisionEntityDataForChild($class, $entityData, $revType)
    {
     ...
     * @return array
     */
    private function extractEntityDataForClass($class, $entityData)
    {
        $entityDataForClass = array();
        foreach ($entityData as $field => $value) {
            if ($this->isClassField($class, $field)) {
                $entityDataForClass[$field] = $value;
            }
        }
        return $entityDataForClass;
    }
